  ...
  onValue(ref(db, `userChats/${me.uid}`), (snap) => {
    ...
    chatIds.forEach((chatId) => {
      ...
      const handler = (metaSnap) => {
        ...
        if (!meta) return;
        const prevLastMsg = chatsCache[chatId]?.lastMessage?.timestamp;
        chatsCache[chatId] = meta;
        renderChatList();
        if (currentChatId === chatId) updateChatHeader(meta);

        // Notify on genuinely new incoming messages
        if (meta.lastMessage && meta.lastMessage.senderId !== me.uid) {
          const ts = meta.lastMessage.timestamp;
          if (ts && ts !== prevLastMsg) maybeNotify(chatId, meta);
        }
      };
...
function setupNotificationPermission() {
  ...
  if (Notification.permission === "default") {
    // Ask once, quietly, after the UI has loaded.
    setTimeout(() => Notification.requestPermission(), 800);
  }
}

function maybeNotify(chatId, meta) {
  if (!("Notification" in window) || Notification.permission !== "granted") return;
  if (document.hasFocus() && currentChatId === chatId) return; // already looking at it
  const name = chatDisplayName(meta);
  const senderName = meta.type === "group" ? (meta.memberNames?.[meta.lastMessage.senderId] || "Someone") : name;
  const body = meta.type === "group" ? `${senderName}: ${meta.lastMessage.text}` : meta.lastMessage.text;
  try {
    const n = new Notification(name, { body, tag: chatId });
    n.onclick = () => { window.focus(); openChat(chatId, meta); };
  } catch (_) { /* some browsers restrict this without a service worker; fail silently */ }
}
